...
// single <script> tag, raw fetch to Anthropic, no SDK dependency added.
//
// Adds one thing the other bots don't have: a photo-intake path. Cosmedix's own
// contact page asks visitors to "send your inquiry and photos of your skin for
// personalised assistance" — every enquiry today is manually reviewed. This bot
// automates that exact flow: a visitor can attach a photo + contact details, and
// it's emailed straight to the clinic via Gmail (see gmail.mjs), instead of the
// bot pretending to assess the photo itself.
import http from "node:http";
import { GMAIL_ENABLED, sendGmailWithAttachment } from "./gmail.mjs";

const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY;
const PORT = process.env.COSMEDIX_CHATBOT_PORT || 3006;
// Deliberately NOT defaulted to the clinic's real address — unset means "log only,
// don't send," so testing never risks emailing Robert a broken test message. Point
// this at your own inbox first, only switch to info@cosmedixclinics.com.au once the
// flow is confirmed working end-to-end.
const NOTIFY_EMAIL = process.env.COSMEDIX_NOTIFY_EMAIL || null;
...
// Anthropic API money (and, for /api/enquiry, a real email send), so cap it per
// visitor rather than leaving it wide open.
const RATE_LIMIT_MAX = 20;
const RATE_LIMIT_WINDOW_MS = 60 * 60 * 1000;
const rateLimitLog = new Map(); // ip -> recent request timestamps

// Photos are personal, sensitive data — cap size and require an image mime type
// so this can't be abused as a free file-upload/email-relay endpoint.
const MAX_PHOTO_BYTES = 6 * 1024 * 1024; // ~6MB raw, comfortably under Gmail's attachment limit
...
- CRITICAL — this is a medical/cosmetic clinic: never diagnose, assess, or comment on the severity of someone's skin, and never tell someone which treatment is right for them or how many sessions they'd need. If someone describes a specific skin concern or asks "what's wrong with..." / "should I get...", tell them the clinic needs to see it in person or via a submitted photo to assess properly — do not speculate.
- If someone describes a specific concern or wants it looked at, tell them they can use the "Send us your photo" option in the chat home screen to have the clinic team review it directly, mirroring the clinic's own stated process of reviewing inquiries and photos personally.
...
(function () {
  ...
  style.textContent =
    ...
    "#cx-chatbar,#cx-formbar{display:flex;align-items:center;gap:8px;padding:12px 16px;border-bottom:1px solid #eaeaea;flex-shrink:0;}" +
    "#cx-back,#cx-form-back{background:none;border:none;color:${DARK};font-size:13px;cursor:pointer;display:flex;align-items:center;gap:4px;padding:2px;font-family:" + FONT + ";}" +
    "#cx-back:hover,#cx-form-back:hover{color:${GOLD_DEEP};}" +
    ...
    "#cx-form{padding:18px 22px;}" +
    "#cx-form p.cx-note{font-size:11.5px;color:#726c63;line-height:1.6;margin:0 0 14px;}" +
    "#cx-form label{display:block;font-size:10.5px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#8a8378;margin:12px 0 4px;}" +
    "#cx-form input[type=text],#cx-form textarea{width:100%;box-sizing:border-box;border:1px solid #e5e0d5;border-radius:2px;padding:9px 10px;font-size:13px;font-family:" + FONT + ";color:${INK};background:${CREAM};}" +
    "#cx-form textarea{min-height:64px;resize:vertical;}" +
    "#cx-form input[type=file]{font-size:11.5px;margin-top:2px;}" +
    "#cx-submit{margin-top:16px;width:100%;border:none;background:${DARK};color:${CREAM};font-weight:700;font-size:12px;letter-spacing:.06em;text-transform:uppercase;padding:12px;cursor:pointer;border-radius:2px;}" +
    "#cx-submit:hover{background:${GOLD_DEEP};}" +
    "#cx-submit:disabled{background:#c9c2b3;cursor:default;}" +
    "#cx-form-status{font-size:12px;margin-top:10px;color:${INK};}" +
  ...
  panel.innerHTML =
    ...
    '<div id="cx-body">' +
      '<div class="cx-view cx-active" id="cx-home">' +
        ...
        '<p>Ask about treatments and conditions we treat, or send us a photo for a personalised look.</p>' +
        '<button class="cx-action" id="cx-go-chat">Ask a question<span class="cx-chev">\\u203A</span></button>' +
        // "Send us your photo" is deliberately hidden for the basic test build —
        // the /api/enquiry endpoint and form view still exist below (harmless if
        // unused), so this is a one-line re-add once Gmail creds are configured.
      ...
      '</div>' +
      '<div class="cx-view" id="cx-form">' +
        '<div id="cx-formbar"><button id="cx-form-back">\\u2039 Back</button></div>' +
        '<div style="padding:0 22px 22px;">' +
          '<p class="cx-note">Send a photo and a short note and our team will personally review it \\u2014 the same process as emailing the clinic directly.</p>' +
          '<label for="cx-f-name">Your name</label><input type="text" id="cx-f-name" />' +
          '<label for="cx-f-contact">Email or phone</label><input type="text" id="cx-f-contact" />' +
          '<label for="cx-f-message">What would you like us to look at?</label><textarea id="cx-f-message"></textarea>' +
          '<label for="cx-f-photo">Photo (optional)</label><input type="file" id="cx-f-photo" accept="image/*" />' +
          '<button id="cx-submit">Send to the clinic</button>' +
          '<div id="cx-form-status"></div>' +
        '</div>' +
      '</div>' +
    '</div>';
  ...
  var formView = panel.querySelector("#cx-form");
  ...
  function showView(view) {
    [homeView, chatView, formView].forEach(function (v) { v.classList.remove("cx-active"); });
    view.classList.add("cx-active");
  }
  launcher.onclick = function () { toggle(); };
  panel.querySelector("#cx-close").onclick = function () { toggle(false); };
  panel.querySelector("#cx-go-chat").onclick = function () {
    showView(chatView);
    setTimeout(function () { input.focus(); }, 200);
  };
  panel.querySelector("#cx-go-form").onclick = function () { showView(formView); };
  panel.querySelector("#cx-back").onclick = function () { showView(homeView); };
  panel.querySelector("#cx-form-back").onclick = function () { showView(homeView); };
  ...
  var submitBtn = panel.querySelector("#cx-submit");
  var statusEl = panel.querySelector("#cx-form-status");
  submitBtn.onclick = async function () {
    var name = panel.querySelector("#cx-f-name").value.trim();
    var contact = panel.querySelector("#cx-f-contact").value.trim();
    var message = panel.querySelector("#cx-f-message").value.trim();
    var fileInput = panel.querySelector("#cx-f-photo");
    var file = fileInput.files[0];

    if (!name || !contact || !message) {
      statusEl.textContent = "Please fill in your name, contact, and a short note.";
    ...
    statusEl.style.color = "${INK}";

    try {
      var payload = { name: name, contact: contact, message: message };
      if (file) {
        payload.photo = {
          filename: file.name,
          mimeType: file.type,
      ...
      var res = await fetch(ENDPOINT + "/api/enquiry", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      statusEl.textContent = "Sent \\u2014 our team will be in touch.";
      statusEl.style.color = "#2f7a4f";
      panel.querySelector("#cx-f-name").value = "";
      panel.querySelector("#cx-f-contact").value = "";
      panel.querySelector("#cx-f-message").value = "";
      fileInput.value = "";
    } catch (e) {
      statusEl.textContent = "Something went wrong \\u2014 please call the clinic instead.";
      statusEl.style.color = "#b03a3a";
    }
...
// Full-screen demo page served at "/" — same widget, always visible (no launcher
// needed for a standalone preview), clearly labeled as a preview build.
...
<main>
  <div class="intro">
    ...
    <p>Click the "Ask a question" button in the bottom right to try it &mdash; grounded in Cosmedix's real treatments and process, answered live. Try "Send us your photo" to see the enquiry-capture flow.</p>
  </div>
</main>
...
function formatEnquiryEmail({ name, contact, message }) {
  return [
    "New enquiry via the Cosmedix Clinics chat concierge (preview build).",
    "",
    `Name: ${name}`,
    `Contact: ${contact}`,
    "",
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/api/enquiry" && req.method === "POST") {
    if (isRateLimited(getClientIp(req))) {
      sendJson(res, 429, { error: "Too many requests — please wait a bit and try again." });
      return;
    }
    logVisit(req, "enquiry-submitted");
    try {
      const { name, contact, message, photo } = await readJsonBody(req);
      if (!name || !contact || !message) {
        sendJson(res, 400, { error: "name, contact, and message are required" });
        return;
      }

      let attachment = null;
      if (photo) {
        if (!photo.mimeType || !ALLOWED_PHOTO_TYPES.has(photo.mimeType)) {
          sendJson(res, 400, { error: "Unsupported photo type" });
          return;
        }
        const approxBytes = (photo.base64Data?.length || 0) * 0.75;
        if (approxBytes > MAX_PHOTO_BYTES) {
          sendJson(res, 413, { error: "Photo is too large — please use a smaller image" });
          return;
        }
        attachment = {
      ...
      if (!GMAIL_ENABLED) {
        console.warn("[enquiry] Gmail not configured — logging instead of sending:", { name, contact, message, hasPhoto: Boolean(attachment) });
      } else if (!NOTIFY_EMAIL) {
        console.warn("[enquiry] COSMEDIX_NOTIFY_EMAIL not set — logging instead of sending:", { name, contact, message, hasPhoto: Boolean(attachment) });
      } else {
        await sendGmailWithAttachment(
          NOTIFY_EMAIL,
          `Cosmedix chat enquiry from ${name}`,
          formatEnquiryEmail({ name, contact, message }),
          attachment
        );
      }
...
server.listen(PORT, () => {
  ...
  console.log(GMAIL_ENABLED ? (NOTIFY_EMAIL ? `Enquiries will email: ${NOTIFY_EMAIL}` : "GMAIL configured but COSMEDIX_NOTIFY_EMAIL not set — enquiries will only be logged.") : "GMAIL not configured — enquiries will only be logged, not emailed.");
});
